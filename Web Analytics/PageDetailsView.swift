    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: Constants.oneColumnLayout, spacing: 20) {
                Graph(title: "Visits", image: "person.2.fill", data: GraphSeries.build(dates: periodsHelper.axisValues(selectedPeriod), apiData: details?.series ?? [], keyPath: \.visits), total: details?.totals.visits ?? 0, prev: details?.totalsPrev.visits ?? 0, color: .blue)
                Graph(title: "Page Views", image: "eye", data: GraphSeries.build(dates: periodsHelper.axisValues(selectedPeriod), apiData: details?.series ?? [], keyPath: \.pageViews), total: details?.totals.pageViews ?? 0, prev: details?.totalsPrev.pageViews ?? 0, color: .orange)
            }
